...
PUBLIC void APP_ZCL_vInitialise(void)
{
   ...
   eZCL_Status =  eZHA_RegisterBaseDeviceEndPoint(ROUTER_APPLICATION_ENDPOINT,
    ...
    eZCL_Status =  eZHA_RegisterBaseDeviceEndPoint(ROUTER_TEMP_MEAS_ENDPOINT,
                                                       &APP_ZCL_cbEndpointCallback,
                                                       &sBaseDeviceTemp);
	if (eZCL_Status != E_ZCL_SUCCESS)
	{
			DBG_vPrintf(TRACE_ZCL, "Error: eZHA_RegisterBaseDeviceEndPoint(TEMP): %02x\r\r\n", eZCL_Status);
	}
...
PRIVATE void APP_vZCL_DeviceSpecific_Init(void)
{
    ...
    sBaseDeviceTemp.sOnOffServerCluster.bOnOff = FALSE;
	FLib_MemCpy(sBaseDeviceTemp.sBasicServerCluster.au8ManufacturerName, "NXP", CLD_BAS_MANUF_NAME_SIZE);
	FLib_MemCpy(sBaseDeviceTemp.sBasicServerCluster.au8ModelIdentifier, "BDB-Router", CLD_BAS_MODEL_ID_SIZE);
	FLib_MemCpy(sBaseDeviceTemp.sBasicServerCluster.au8DateCode, "20150212", CLD_BAS_DATE_SIZE);
